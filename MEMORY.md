...
| Rama principal | `main` |
| Rama de trabajo actual | Ninguna — `main` al día (PR #12 fusionado; la rama `feature/modelos-datos-dynamodb-client` ya fue eliminada) |
| Última sesión | 2026-07-18 — PR #12 fusionado (modelos de datos + cliente DynamoDB); después: verificación agnóstica de plataforma en CI (smoke test post-deploy, workflow manual de operaciones sobre `staging`, secrets de negocio inyectados también en `staging`) y decisión de no usar la fuente Angellya en desarrollo (ver §9) |
...
- [x] Modelos de datos compartidos + cliente DynamoDB base (PR #12, rama `feature/modelos-datos-dynamodb-client`) — ver detalle en §9. Pendiente: verificación en vivo contra `staging` (ver aviso en `TODO.md`).
...
**Secret pendiente de crear por el usuario: `FIREBASE_SERVICE_ACCOUNT_BABEL` (bloqueante para `TODO.md` Tarea 1).** El 2026-07-18 se le dieron al usuario las instrucciones de navegador para crear la cuenta de servicio propia de Babel (sugerida: `babel-backend@comandante-letiende.iam.gserviceaccount.com`, rol mínimo `Firebase Authentication Viewer`) en el proyecto GCP compartido `comandante-letiende` (nunca reutilizar la de Comandante — ADR-007) y guardar el JSON completo como secret de GitHub Actions. El job `desplegar-staging` ya inyecta este secret (y los demás secrets de negocio) al deploy; mientras el secret no exista, `serverless.yml` lo resuelve a `''` sin romper el deploy.

**Verificación agnóstica de plataforma (2026-07-18):** dos mecanismos en GitHub Actions para que cualquier sesión (CLI, Web, Mobile, Desktop) pueda verificar contra `staging` real sin credenciales AWS locales:
...
2. **Workflow manual `operaciones-staging.yml`** (`workflow_dispatch`): siembra/consulta/elimina usuarios de prueba en `babel-usuarios-staging` usando los secrets de AWS del repo. Se dispara desde la UI de GitHub (Actions → "Operaciones sobre staging" → Run workflow) o con `gh workflow run operaciones-staging.yml -f operacion=... -f email=...`. Diseñado para el Definition of Done de la Tarea 1 (crear/limpiar el registro de prueba); agregar operaciones nuevas ahí mismo cuando otra tarea necesite sembrar otras tablas.
...
**Próxima tarea sugerida:** ver `TODO.md` — Tarea 1 (verificación real del ID Token de Firebase + resolución de rol; requiere que el usuario cree antes el secret `FIREBASE_SERVICE_ACCOUNT_BABEL`, ver §5) y Tarea 2 (endpoint público `GET /api/libros`, primera pieza que ejercita de verdad el cliente DynamoDB contra datos reales). Ambas son independientes entre sí.
