...
| Rama principal | `main` |
| Rama de trabajo actual | `feature/login-component` (PR #6, sin fusionar) |
| Última sesión | 2026-07-17 — `LoginComponent` + `NoAuthGuard`, probado con deploy real a `staging` (bug de `RenderMode.Prerender` encontrado y corregido) |
...
- [x] `LoginComponent` + `NoAuthGuard` — probado con deploy real a `staging` (PR #6, rama `feature/login-component`) — ver detalle en §7 y §9. Pendiente: `RoleGuard` y verificación de rol en backend.
