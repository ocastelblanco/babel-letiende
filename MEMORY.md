...
Pendiente (sin configurar en esta sesión): dominio personalizado `babel.letiende.co` en Route53/API Gateway/ACM (fuera de alcance de la tarea de esqueleto — ver roadmap técnico), y el secret `SERVERLESS_ACCESS_KEY` en GitHub Actions (requerido por la CLI v4 de Serverless Framework para operar en CI, no es un secreto del dominio de Babel — ver `.github/workflows/deploy.yml` y PR #3).
