...
- [x] Backend (2026-08-19, rama `feature/validar-libros-backend`): tabla `babel-validaciones-libros`, 3 funciones Lambda (`handlerIniciar`, `handlerWorker`, `handlerConsultar` en `server/api/handlers/validaciones-libros.ts`) + roles IAM (incluida la auto-invocación con ARN por convención) + tests unitarios (28 nuevos: consenso de PVP, agrupación por mueble, portada pendiente vs. corregida, auto-invocación, corridas colgadas). Nueva dependencia `@aws-sdk/client-lambda`. `npm run build:api`, `npm run build` y `npx serverless package --stage dev` (packaging real, sin desplegar) verificados sin errores. Pendiente de PR + revisión del usuario + validación en `staging`.
- [ ] Frontend: `ValidarLibrosComponent` en `/admin/validar-libros` + polling + tests. Arranca cuando el backend esté fusionado y desplegado en `staging` (regla dura de staging compartido, `MEMORY.md` de conversación).
