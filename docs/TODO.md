...
**Diseño detallado — COMPLETO (2026-08-19, este PR):** el detalle fino que no estaba persistido en ningún documento (modelo exacto de `babel-validaciones-libros`, contrato de las 3 Lambdas, permisos IAM incluida la auto-invocación, tamaño de lote, forma del polling desde el frontend) quedó resuelto en `docs/plan-validar-libros-async.md`, con ADR-012 nuevo en `docs/MEMORY.md` §3 justificando la arquitectura (sin Step Functions/SQS) y referencias agregadas en `docs/tech-specs.md` (§5, §5.1, §11) y `docs/PRD.md` (§5.6, §6).

- [ ] Backend: tabla `babel-validaciones-libros` + 3 funciones Lambda (`handlerIniciar`, `validarLibrosWorker`, `handlerConsultar`) + roles IAM + tests unitarios (consenso de PVP, agrupación por mueble). Puede ir en su propia rama/PR si el frontend no cabe en el mismo.
- [ ] Frontend: `ValidarLibrosComponent` en `/admin/validar-libros` + polling + tests.

Pendiente iniciar la implementación (empieza al día siguiente de fusionar este PR de diseño, mismo ritmo ya usado en el proyecto para features grandes — ver `MEMORY.md` de conversación, "Flujo de PR por tarea").
