...
Con esto se cierra el PR A. Tras fusionar y desplegar a `staging`/`production` (el GSI se crea vacío, sin backfill automático de lo ya existente), sigue el **backfill** (el agente lo corre con el MCP de AWS: primero `staging`, verificado con el usuario, luego `production` con su confirmación explícita) y solo después la **Tarea 5 (PR B)**: cambiar `GET /api/libros` de `Scan` a `Query` sobre `disponible-index`.
