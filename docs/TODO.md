...
**Lote nuevo (2026-09-17) — Rendimiento de la carga del catálogo:** el usuario reportó que el catálogo (pantalla inicial y Catalogar > Editar) tarda demasiado en cargar. Se investigó el código real con un agente de exploración antes de escribir las tareas — causa **estructural**, no coyuntural: `GET /api/libros` y `GET /api/libros/inventario` hacen `Scan` completo de `babel-libros` (3.000+ libros) sin paginación hacia el cliente, sin índice que calce con "disponibles"/"por ubicación", y el frontend renderiza el arreglo completo sin virtual scrolling. Detalle completo, hallazgos y las 5 tareas atómicas ordenadas por impacto en **`docs/plan-rendimiento-catalogo.md`** (nuevo). Se saca primero la Tarea 1 (virtual scrolling, puramente frontend, sin riesgo de backend) — las Tareas 2-5 quedan en backlog, con una pregunta abierta de diseño sobre paginación vs. búsqueda en memoria documentada en el plan antes de detallarlas.

## Tarea 1 — Virtual scrolling en Catálogo público y Catalogar > Editar — COMPLETA (2026-09-17)

**Decisión técnica tomada antes de implementar:** la vista "Tarjetas" del catálogo público es una grilla responsive (2/3/4 columnas según breakpoint de Tailwind) — `cdk-virtual-scroll-viewport` (`@angular/cdk/scrolling`) asume una sola columna y no calza bien ahí sin trabajo extra de chunking manual por fila. Se optó por **renderizado incremental (windowing) con `IntersectionObserver`**, uniforme para las 3 listas (grilla de tarjetas, lista del catálogo público, lista de Catalogar > Editar): un signal `limiteRenderizado` (inicial 60) que solo crece cuando un elemento centinela al final de la lista/grilla entra en el viewport, hasta cubrir el total. Mantiene el scroll nativo de `window` (sin contenedor de scroll propio), sin conflicto con el botón flotante "Volver arriba" (tarea del 2026-09-04). Se descartó `@angular/cdk/scrolling` para esta tarea (sí sigue en uso para `DragDropModule`, sin relación).

- [x] Nueva directiva compartida standalone `ScrollInfinitoDirective` (`src/app/shared/directivas/scroll-infinito.directive.ts`, selector `appScrollInfinito`) — `output()` `alcanzarFinal` que dispara cuando el elemento host intersecta el viewport (`IntersectionObserver`, SSR-safe con `isPlatformBrowser`), mismo patrón que `SinPortadaFallbackDirective`. 5 tests nuevos con mock de `IntersectionObserver`.
- [x] `EditarLibroComponent`: signal `limiteRenderizado` (tanda de 60, `effect()` resetea al cambiar `librosFiltrados()`), computed `librosVisibles`, `@for` de `editar-libro.component.html:54` sobre `librosVisibles()`, centinela `<li appScrollInfinito>` al final del `<ul>` (visible solo mientras queden libros por renderizar) que llama `cargarMasLibros()`. 3 tests nuevos.
- [x] `CatalogoPublicoComponent`: mismo patrón sobre `librosOrdenados()` → `librosVisibles`, aplicado a la vista Tarjetas y a la vista Lista (el reset al cambiar `librosOrdenados()` ya cubre búsqueda/filtro de espacio-mueble/orden, que recalculan ese mismo computed). 7 tests nuevos, incluida una prueba de regresión de que "Volver arriba" sigue funcionando.
...
Con esto se cierra la Tarea 3. La Tarea 2 (paginación real) sigue en backlog, a la espera de que el usuario mida en `staging` si el payload liviano ya es suficiente. **Confirmado por el usuario en `staging`: "Todo funciona bien", PR #132 fusionado.**

**Prioridad de selección aplicada (2026-09-18):** el usuario pidió continuar con la Tarea 5 (GSI disperso por `cantidadDisponible`), saltándose la Tarea 4 (N+1 de `resolverEjemplares`, que queda en backlog). Diseño completo en `docs/plan-rendimiento-catalogo.md` §3 — **2 PRs separados + un backfill real en medio**, no una sola tarea, porque los 3.000+ libros ya existentes no tienen el atributo nuevo hasta que se les haga backfill (cambiar `Scan`→`Query` antes de eso vaciaría el catálogo público en producción). El usuario decidió que el agente corre el backfill directamente con el MCP de AWS, con confirmación explícita antes de tocar `production`.

## Tarea 5 (PR A) — Atributo disperso `disponibleParaCatalogo` + GSI nuevo, sin cambiar `GET /api/libros` todavía — COMPLETA (2026-09-18)

Ver `docs/plan-rendimiento-catalogo.md` §3 para el diseño completo. Este PR NO cambia el comportamiento de `GET /api/libros` (sigue en `Scan`) — solo prepara el atributo/índice y lo mantiene en escritura, para poder hacer el backfill después con seguridad.

- [x] `serverless.yml`: nuevo `AttributeDefinition` (`disponibleParaCatalogo`, `S`) + nuevo GSI `disponible-index` (`KeySchema` HASH sobre ese atributo, `ProjectionType: ALL`) en `TablaLibros`. Sin `ProvisionedThroughput` (hereda `PAY_PER_REQUEST` de la tabla, mismo criterio que `isbn-index`). Confirmado en el CloudFormation real (`serverless print --stage dev`).
- [x] `handlerCrear`/`handlerEditar` (`libros.ts`): el ítem a persistir incluye `disponibleParaCatalogo: 'SI'` solo si `cantidadDisponible > 0` — construido aparte del objeto `Libro` de la respuesta HTTP, que nunca lo incluye.
- [x] `fusionarLibroDuplicado` (`dynamodb.ts`): `SET #disponibleParaCatalogo = :disponibleParaCatalogo` incondicional en el `UpdateExpression` existente (el resultado de la fusión siempre queda `cantidadDisponible > 0`). **Hallazgo real durante la revisión (no anticipado en el diseño):** esta función devuelve el ítem `ALL_NEW` tal cual queda en DynamoDB, así que el campo nuevo se filtraba a la respuesta HTTP de `POST /api/libros/:bookId/fusionar-duplicado` — corregido en `handlerFusionarDuplicado` descartándolo explícitamente antes de responder (mismo criterio que `handlerCrear`/`handlerEditar`), con un test de regresión dedicado que lo prueba con el campo presente en el mock.
- [x] `decrementarPorCantidadSiSuficiente` (`dynamodb.ts`, venta): `ReturnValues: 'UPDATED_NEW'` para conocer el valor real post-decremento (nunca calculado localmente); nuevo tipo `ResultadoDecremento` (`{ exito: true; nuevoValor } | { exito: false }`). Si `nuevoValor === 0`, `ventas.ts` llama a la nueva función `removerAtributo` (`REMOVE` puntual) sobre ese `bookId`.
- [x] Tests nuevos: 12 en total (`dynamodb.spec.ts`: decremento con/sin agotar stock, `removerAtributo`, `fusionarLibroDuplicado`; `libros.spec.ts`: crear/editar con y sin el atributo, y el test de regresión de la fuga en fusionar-duplicado; `ventas.spec.ts`: venta que agota vs. no agota stock). 405 tests backend (antes 393).
- [x] `npm run build:api`, `npm run test:api`, `npx serverless print --stage dev` y `npx serverless package --stage dev` verificados en verde, de forma independiente (no solo el reporte del agente — la fuga de `fusionarLibroDuplicado` se encontró precisamente en esta revisión).
- [x] `GET /api/libros` sin tocar, confirmado — sigue en `Scan`, sin cambio de comportamiento.

Con esto se cierra el PR A. **PR #133 fusionado, desplegado a `production` sin novedad (confirmado por el usuario, 2026-09-18).**

**Backfill — COMPLETO (2026-09-18):** corrido por el agente con el MCP de AWS. `staging` (`babel-libros-staging`, 22 libros): 1 sola pasada, 22/22 correctos, 0 errores. `production` (`babel-libros-production`, **1.000 libros exactos**, confirmado con `Scan` sin `LastEvaluatedKey`), con confirmación explícita del usuario antes de tocar datos reales: 900 candidatos, procesados en 10 lotes de hasta 99 escrituras (el sandbox de ejecución de scripts limita a 100 llamadas a AWS por ejecución — 1 `Scan` + hasta 99 `UpdateItem`, repetido de forma idempotente hasta agotar los pendientes), verificado al final con un escaneo de solo lectura: 1.000/1.000 correctos, 0 inconsistencias. Detalle completo de la técnica de backfill por lotes documentado en `docs/MEMORY.md` §7 (gotcha nuevo) para reutilizar en tareas futuras similares.

Con el backfill confirmado en ambos entornos, sigue la **Tarea 5 (PR B)**: cambiar `GET /api/libros` de `Scan` a `Query` sobre `disponible-index`.

## Tarea 5 (PR B) — `GET /api/libros` de `Scan` a `Query` sobre `disponible-index` — COMPLETA (2026-09-18)

- [x] `server/api/handlers/libros.ts`, `handler` (`GET /api/libros`): reemplaza `escanearMayorQue<LibroIndice>(...)` por `consultarPorIndice<LibroIndice>(nombreTablaLibros(), 'disponible-index', 'disponibleParaCatalogo', 'SI', [...8 campos...])`, manteniendo la misma proyección (Tarea 3) y la misma restitución de `isbn ?? null`.
- [x] `consultarPorIndice` (`dynamodb.ts`) generalizada: ahora pagina todas las páginas de un `Query` (mismo límite de ~1 MB por página que un `Scan`, mismo criterio que `escanearPaginado` — evita reintroducir el bug de producción del 2026-08-19 documentado en `MEMORY.md` §7) y acepta proyección opcional (mismo mecanismo que `escanearMayorQue`), sin cambiar el comportamiento de sus 2 llamadores existentes (`isbn-index`).
- [x] IAM: **hallazgo real durante la implementación** — `LibrosLambdaRole` es compartido por `libros` (este handler) y `librosSitemap` (que SIGUE haciendo `Scan` completo, fuera de alcance de esta tarea). Se agregó `dynamodb:Query` sobre `${TablaLibros.Arn}/index/disponible-index` **manteniendo** el `dynamodb:Scan` existente (no reemplazándolo, como decía la redacción original de esta tarea) — quitarlo habría roto `librosSitemap` con `AccessDeniedException` en producción. Verificado en el CloudFormation real (`serverless print --stage dev`).
- [x] Tests actualizados: los 3 tests de `handler` (Tarea 3) ahora mockean `consultarPorIndice` en vez de `escanearMayorQue`, verificando la llamada exacta a `disponible-index`. 7 tests nuevos de `consultarPorIndice` (paginación, proyección, regresión sin proyección). 412 tests backend (antes 405).
