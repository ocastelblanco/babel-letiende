...
- [ ] Frontend — LISTO PARA INICIAR: `ValidarLibrosComponent` en `/admin/validar-libros` (`RoleGuard('administrador')`) + polling cada 3s + tests, según el contrato ya definido en `docs/plan-validar-libros-async.md` §7 (botón "Iniciar validación" deshabilitado mientras haya una corrida `en_progreso`, barra de progreso `librosRevisados/totalLibros`, `muebleActualNombre`, contadores de PVP/portada en vivo, lista final de `portadasPendientes` con link a Editar, lista de `erroresLibro`). Backend ya desplegado y validado en `staging` — sin bloqueos. Nueva rama `feature/validar-libros-frontend` desde `main`. Con este PR se cierra por completo la Tarea 3 y el lote de 3 ajustes de catalogación traído el 2026-08-18.
