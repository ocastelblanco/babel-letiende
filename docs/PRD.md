...
→ Sistema busca los datos del libro (título, autor, editorial, portada) por ISBN o por título/autor
   ...
   ├─ 2.º si faltan datos: scraping de los sitios autorizados para «info» (lista única, por orden de prioridad) — una portada extraída que coincide con alguna «palabra clave inválida» configurada para ese sitio (placeholder tipo "no disponible") se descarta y se prueba el siguiente sitio de la cola
...
| **Gestionar** | Área de la aplicación para vendedor y administrador con las dos operaciones del día a día: Catalogar y Editar libros ya catalogados. Distinta de la sección de Administración, exclusiva del administrador. El link/ruta visible en el header se llama "Catalogar" (`/catalogar`), pero el componente y la sección interna se llaman "Gestionar" (desacoplamiento intencional para mantener la coherencia con la nomenclatura actual del código). |
...
| **Palabras clave inválidas (scraping)** | Lista de palabras, configurable por sitio de scraping, que identifican una portada placeholder (ej. "no disponible", "sin imagen") que ese sitio devuelve como si fuera una portada real. Si la URL de la portada extraída contiene alguna, el sistema la descarta y prueba el siguiente sitio de la cola de prioridad — nunca la pre-carga como si fuera válida. |
