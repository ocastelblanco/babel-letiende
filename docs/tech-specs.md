...
| **Base de datos** | AWS DynamoDB | — | Almacenamiento NoSQL de libros, ventas, estantes, usuarios y descuentos; modo **on-demand (`PAY_PER_REQUEST`)** — nunca aprovisionado, ver §7.3. | [DynamoDB](https://aws.amazon.com/dynamodb) |
...
| GET | `/api/metadatos/:isbn/portadas` | Vendedor/Admin | Selector manual de portada (`TODO.md`): lista TODAS las portadas candidatas encontradas en vivo en los sitios `info: true` para el ISBN (no solo la primera por prioridad, a diferencia de `GET /api/metadatos/:isbn`), excluyendo las que coinciden con `palabrasClaveInvalidas` de su sitio de origen. Devuelve `{ portadas: [{ dominio, nombre, portadaUrl }] }`, ordenado por `prioridad`; siempre `200`. | — |
...
| Sitios de scraping (datos + PVP) | Semilla inicial: 4 sitios; 6 con adaptador de código a 2026-08-20 (+ Casa Tomada, Librería de la U) | Fuente de datos bibliográficos y de PVP por ISBN. **Lista única administrable desde la UI de administración**, almacenada en `babel-sitios-scraping` (ADR-010) — supersede la decisión previa de mantenerla como configuración estática en el repo. Cada sitio declara permisos `info`/`pvp`. La mecánica de extracción por sitio (URL de búsqueda + selectores) vive en código (`scraping.ts`); toda petición saliente pasa por la guardia SSRF fija (ADR-011). **Agregar un sitio en la UI solo declara política — sin un adaptador de código para su dominio en `ADAPTADORES_POR_DOMINIO`, el sitio se registra y se omite en silencio (sin error visible), ver ADR-010.** |
...
- **DynamoDB:** modo **on-demand (`PAY_PER_REQUEST`)** en todas las tablas. **Nunca capacidad aprovisionada.**
  > ⚠️ **Corregido el 01/08/2026 tras un sobrecosto real de US$90 en julio.** La versión anterior de esta línea indicaba "capacidad aprovisionada 25 RCU/25 WCU por tabla (capa siempre gratuita), evitando el modo on-demand". Era **falso** y costó dinero: el límite gratuito de 25 RCU + 25 WCU es **por cuenta, no por tabla**. Con 8 tablas × 2 stages + 2 GSIs se aprovisionaron 450 RCU + 450 WCU (18× el límite), facturados 24/7 a US$8,42/día ≈ US$256/mes aunque nadie usara la aplicación. A esto se suma que AWS retiró el "always free" de DynamoDB en 2025 para cuentas nuevas. Ver `docs/advertencia-urgente-costos-aws.md`.
